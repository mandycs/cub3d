...
/*   Updated: 2025/02/27 18:57:30 by ribana-b         ###   ########.com      */
/*                                                                            */
/* ************************************************************************** */
...
void	render(void *param)
{
	...
	for (double a = 0; a < info->player.fov * 0.5; a += 0.5)
	{
		new_cast_ray(&info->screen, &info->map, &info->player, fmod(info->player.angle + a, 360) * DEG2RAD);
		new_cast_ray(&info->screen, &info->map, &info->player, fmod(info->player.angle - a, 360) * DEG2RAD);
	}
